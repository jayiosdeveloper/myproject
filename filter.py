...
Original file is located at
    https://colab.research.google.com/drive/19p8zcslVnB_QRByf9_ycQ4cpX2Ua0mWi
"""

!pip install fastapi uvicorn opencv-python pillow nest_asyncio pyngrok
...
def apply_invert_blue(img):
    img = img.copy()
    img[:,:,0] = 255 - img[:,:,0]  # Invert blue channel
    return img
...
def apply_halftone(img):
    ...
    for y in range(0, gray.shape[0], radius*2):
        for x in range(0, gray.shape[1], radius*2):
            ...
            if circle_radius > 0:
                cv2.circle(halftone, (x+radius, y+radius), circle_radius,
                          (255, 255, 255), -1)

    return halftone

def apply_thermal(img):
    # Apply colormap for thermal camera effect
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
...
def apply_night_vision(img):
    ...
    green_tint[:,:,1] = 255  # Set green channel to maximum

    # Blend with original image
    return cv2.addWeighted(img, 0.7, green_tint, 0.3, 0)

def apply_cyanotype(img):
    # Blueprint/cyanotype effect
    blue_filter = np.array([[[0.7, 0.5, 0.2]]], dtype=np.float32)  # Blue dominant
    cyanotype = cv2.transform(img, blue_filter)
    return np.clip(cyanotype, 0, 255).astype(np.uint8)

def apply_gotham(img):
    # Gotham filter (dark blue tone with contrast)
    b, g, r = cv2.split(img)

    # Increase blue and red, decrease green
...
COLOR_FILTERS = [(40,0,0),(0,40,0),(0,0,40),(40,40,0),(0,40,40),(40,0,40),(60,30,0),(0,60,30),(30,0,60),(50,50,50)]
LUTS = {f"gamma_{g}":build_lut(g/10.0) for g in range(5,30)}
...
FILTERS = {
    ...
    "edge_enhance": apply_edge_enhance, "hue_shift": apply_hue_shift,
    "saturation_boost": apply_saturation_boost, "invert_blue": apply_invert_blue,
    ...
    "saturation_boost": apply_saturation_boost,
    "invert_blue": apply_invert_blue,
    "red_boost": apply_red_boost,
    "green_boost": apply_green_boost,
    ...
    "cyanotype": apply_cyanotype,
    "gotham": apply_gotham,
    "clarity": apply_clarity,
    "dramatic_bw": apply_dramatic_bw,
    "foggy": apply_foggy,
    "golden_hour": apply_golden_hour,
}

# Remove previous dynamic filters
# for idx,(r,g,b) in enumerate(COLOR_FILTERS):
#     FILTERS[f"color_shift_{idx+1}"] = lambda img,r=r,g=g,b=b: color_shift(img,r,g,b)
# for name,lut in LUTS.items():
#     FILTERS[name] = lambda img,lut=lut: lut_filter(img,lut)
...
#https://image-enchantress-ai.lovable.app
...
!ngrok config add-authtoken "31UzZzfYmvNag9VHfpMYajXmmAC_2Va1LxPQXocBTQ9MXDkV8"
nest_asyncio.apply()
